...
module Yast
  class ProxyProposalClient < Client
    def main
      ...
      Builtins.y2milestone("Proxy proposal started")
      ...
      # run the module
        ...
        def TestProxyReturnCode(test_type, proxy_ret_stderr)
          ...
          Builtins.foreach(Builtins.splitstring(proxy_ret_stderr, " ?\n")) do |proxy_stderr|
            if Builtins.regexpmatch(proxy_stderr, "HTTP/[0-9.]+ [0-9]+")
              proxy_retcode = Builtins.regexpsub(
                proxy_stderr,
                ".*(HTTP.*)",
                "\\1"
              )
        ...
        def QueryWidgets
          @same_proxy = Convert.to_boolean(
            UI.QueryWidget(Id(:same_proxy), :Value)
          )
          ...
          while true
            ...
            # next
              ...
              if @user == "" && @pass != ""
                ...
                Popup.Error(
                  _(
                    "You cannot enter a password and leave the user name empty."
                  )
                )
                UI.SetFocus(Id(:user))
                next
              ...
              if @http != "" && @http != @prefill
                ...
                if Ops.get_string(urlmap, "scheme", "") == ""
                  ...
                  Popup.Error(
                    _(
                      "HTTP proxy URL must contain a scheme specification (http)."
                    )
                  )
                  UI.SetFocus(Id(:http))
                  next
              ...
              if @ftp != "" && @ftp != @prefill
                ...
                if Ops.get_string(urlmap, "scheme", "") == ""
                  ...
                  Popup.Error(
                    _(
                      "FTP proxy URL must contain a scheme specification (http)."
                    )
                  )
                  UI.SetFocus(Id(:ftp))
                  next
        ...
        # main ui function
        @result = Convert.to_symbol(ProxyMainDialog(true))

        UI.CloseDialog

        Proxy.Import(@stored) if @result != :next
      ...
      Builtins.y2milestone("Proxy proposal finished")
      Builtins.y2milestone("----------------------------------------")
      deep_copy(@ret) 
...
Yast::ProxyProposalClient.new.main
